...
class Mix120Flowers(Dataset):
    """
    120-class FSCIL dataset built from build_mix120flowers.py:
      - 100 MiniImageNet classes + 20 Oxford 102 Flowers classes
      - Base session: 60 classes (52 mini + 8 flowers), many-shot
      - 12 incremental sessions of 5-way (4 mini + 1 flower each), 5-shot

    Directory layout expected under `root/mix120flowers/`:
      images/<cid>/<cid>__<filename>
      split/train.csv, split/test.csv    (rows: filename,cid -- filename already
                                           includes the cid__ prefix written by
                                           build_mix120flowers.py)
      index_list/session_1.txt ... session_13.txt
                                          (session_1 = base, docs only;
                                           2..13 = the 12 incremental sessions)
      class_order.txt                    (cid <TAB> global_label, 0..119)

    Identical loading logic to Mix120/Mix120Dogs/Mix200/Dogs120/Flowers102 --
    only the root subfolder name differs.
    """
    ...
    def __init__(self, root='./dataset', train=True, index_path=None, index=None,
        ...
        self.MIX_ROOT = osp.join(self.root, 'mix120flowers')
        self.IMAGE_PATH = osp.join(self.MIX_ROOT, 'images')
        self.SPLIT_PATH = osp.join(self.MIX_ROOT, 'split')
        self.INDEX_PATH = osp.join(self.MIX_ROOT, 'index_list')
        ...
        # global cid -> label map, fixed by build_mix120flowers.py so it's identical
        # across every process/run (base session, every incremental session, and test)
        self.label_of = {}
        with open(osp.join(self.MIX_ROOT, 'class_order.txt'), 'r') as f:
            for line in f:
                cid, lb = line.strip().split('\t')
                self.label_of[cid] = int(lb)
        ...
        for line in lines:
            ...
            path = osp.join(self.IMAGE_PATH, cid, fname)
    ...
    def SelectfromTxt(self, index_path):
        ...
        '<cid>/<cid>__<filename>' -- robust to nesting, unlike a positional split()."""
