...
def set_up_datasets(args):
    ...
    if args.dataset == 'mini_imagenet':
        ...
        args.shot = 5
        args.sessions = 9
    if args.dataset == 'mix120':
        import dataloader.mix120.mix120 as Dataset
        args.base_class = 60
        args.num_classes = 120
        args.way = 5
        args.shot = 5
        args.sessions = 13   # 1 base + 12 incremental
    if args.dataset == 'mix200':
        import dataloader.mix200.mix200 as Dataset
        args.base_class = 100
        args.num_classes = 200
        args.way = 10
        args.shot = 5
        args.sessions = 11
    if args.dataset == 'cub120dom':
        import dataloader.cub120dom.cub120dom as Dataset
        args.base_class = 60
        args.num_classes = 120
        args.way = 5
        args.shot = 5
    ...
    if args.dataset == 'flowers102':
        import dataloader.flowers102.flowers102 as Dataset
        args.base_class = 62
        args.num_classes = 102
        args.way = 5
        args.shot = 5
        args.sessions = 9   # 1 base + 8 incremental
    if args.dataset == 'dogs120':
        import dataloader.dogs120.dogs120 as Dataset
        args.base_class = 60
        args.num_classes = 120
        args.way = 5
        args.shot = 5
        args.sessions = 13   # 1 base + 12 incremental
    if args.dataset == 'timgnet200':
        import dataloader.timgnet200.timgnet200 as Dataset
        args.base_class = 100
        args.num_classes = 200
        args.way = 10
        args.shot = 5
        args.sessions = 11   # 1 base + 10 incremental -- mirrors cub200's config
    if args.dataset == 'mix120dogs':
        import dataloader.mix120dogs.mix120dogs as Dataset
        args.base_class = 60
        args.num_classes = 120
        args.way = 5
        args.shot = 5
        args.sessions = 13   # 1 base + 12 incremental
    if args.dataset == 'mix120flowers':
        import dataloader.mix120flowers.mix120flowers as Dataset
        args.base_class = 60
        args.num_classes = 120
        args.way = 5
        args.shot = 5
...
#    txt_path = "/dataset/index_list/" + args.dataset + "/session_" + str(0 + 1) + '.txt'4
    txt_path = os.path.join(args.dataroot, args.dataset, "index_list", "session_" + str(0 + 1) + '.txt')
    ...
    if args.dataset == 'mix120':
        trainset = args.Dataset.Mix120(root=args.dataroot, train=True,
                                        index=class_index, base_sess=True,
                                        crop_transform=crop_transform, secondary_transform=secondary_transform)
        testset = args.Dataset.Mix120(root=args.dataroot, train=False, index=class_index)

    if args.dataset == 'mix200':
        trainset = args.Dataset.Mix200(root=args.dataroot, train=True,
                                        index=class_index, base_sess=True,
                                        crop_transform=crop_transform, secondary_transform=secondary_transform)
        testset = args.Dataset.Mix200(root=args.dataroot, train=False, index=class_index)

        testset = args.Dataset.Dogs120(root=args.dataroot, train=False, index=class_index)

    if args.dataset == 'timgnet200':
        trainset = args.Dataset.TinyImageNet200(root=args.dataroot, train=True,
                                                 index=class_index, base_sess=True,
                                                 crop_transform=crop_transform, secondary_transform=secondary_transform)
        testset = args.Dataset.TinyImageNet200(root=args.dataroot, train=False, index=class_index)


    if args.dataset == 'mix120flowers':
        trainset = args.Dataset.Mix120Flowers(root=args.dataroot, train=True,
                                               index=class_index, base_sess=True,
                                               crop_transform=crop_transform, secondary_transform=secondary_transform)
        testset = args.Dataset.Mix120Flowers(root=args.dataroot, train=False, index=class_index)
...
def get_new_dataloader(args,session):
    ...
    txt_path = os.path.join(args.dataroot, args.dataset, "index_list", "session_" + str(session + 1) + '.txt')
    if args.dataset == 'cifar100':
    ...
    if args.dataset == 'mini_imagenet':
        trainset = args.Dataset.MiniImageNet(root=args.dataroot, train=True, index_path=txt_path,
                                             crop_transform=crop_transform, secondary_transform=secondary_transform)
    if args.dataset == 'mix120':
        trainset = args.Dataset.Mix120(root=args.dataroot, train=True, index_path=txt_path,
                         crop_transform=crop_transform, secondary_transform=secondary_transform)
    if args.dataset == 'mix200':
        trainset = args.Dataset.Mix200(root=args.dataroot, train=True, index_path=txt_path,
                         crop_transform=crop_transform, secondary_transform=secondary_transform)
    if args.dataset == 'timgnet200':
        trainset = args.Dataset.TinyImageNet200(root=args.dataroot, train=True, index_path=txt_path,
                         crop_transform=crop_transform, secondary_transform=secondary_transform)
    if args.dataset == 'mix120flowers':
        trainset = args.Dataset.Mix120Flowers(root=args.dataroot, train=True, index_path=txt_path,
                         crop_transform=crop_transform, secondary_transform=secondary_transform)
    ...
    if args.dataset == 'mini_imagenet':
        testset = args.Dataset.MiniImageNet(root=args.dataroot, train=False,
                                      index=class_new)
    if args.dataset == 'mix120':
        testset = args.Dataset.Mix120(root=args.dataroot, train=False,index=class_new)
    if args.dataset == 'mix200':
        testset = args.Dataset.Mix200(root=args.dataroot, train=False,index=class_new)
    if args.dataset == 'timgnet200':
        testset = args.Dataset.TinyImageNet200(root=args.dataroot, train=False, index=class_new)
    if args.dataset == 'mix120flowers':
        testset = args.Dataset.Mix120Flowers(root=args.dataroot, train=False, index=class_new)
