...
class Mix200(Dataset):
    """
    200-class FSCIL dataset built from build_mix200.py:
      - 140 CUB200 (dominant) classes + 60 MiniImageNet (minority) classes
      - Standard CUB FSCIL geometry: 100 base classes, 10 incremental
        sessions of 10-way, 5 images/class each
      - Base session: 50 CUB + 50 MiniImageNet classes (many-shot)
      - Each incremental session: 9 CUB + at most 1 MiniImageNet class (5-shot)

    Directory layout expected under `root/mix200/`:
    ...
    def __init__(self, root='./dataset', train=True, index_path=None, index=None,
        ...
        self.DOM_ROOT = osp.join(self.root, 'mix200')
        self.IMAGE_PATH = osp.join(self.DOM_ROOT, 'images')
        self.SPLIT_PATH = osp.join(self.DOM_ROOT, 'split')
        self.INDEX_PATH = osp.join(self.DOM_ROOT, 'index_list')
        ...
        # global wnid -> label map, fixed by build_mix200.py so it's identical
        # across every process/run (base session, every incremental session, and test)
        self.label_of = {}
        with open(osp.join(self.DOM_ROOT, 'class_order.txt'), 'r') as f:
            for line in f:
        ...
        if train:
            # CUB-style FSCIL setups commonly train at 224px (unlike the 84px
            # MiniImageNet convention used by Mix120); adjust if your backbone
