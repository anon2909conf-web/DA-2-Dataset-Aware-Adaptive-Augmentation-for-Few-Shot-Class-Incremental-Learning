...
class Mix120(Dataset):
    """
    120-class FSCIL dataset built from build_mix120.py:
      - 60 base classes + 40 novel classes from MiniImageNet (labels 0-99)
      - 20 novel classes from CUB200                          (labels 100-119)
      - 12 incremental sessions of 5-way, 5 images/class each

    Directory layout expected under `root/mix120/`:
      ...
      split/train.csv, split/test.csv    (rows: filename,wnid -- filename already
                                           ...
                                           build_mix120.py)
      index_list/session_1.txt ... session_12.txt
                                          (rows: "<wnid>/<wnid>__<filename>")
      class_order.txt                    (wnid <TAB> global_label, 0..119)
    """
    ...
    def __init__(self, root='./dataset', train=True, index_path=None, index=None,
        ...
        self.MIX_ROOT = osp.join(self.root, 'mix120')
        ...
        # global wnid -> label map, fixed by build_mix120.py so it's identical
